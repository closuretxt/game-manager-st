...
const CHAR_RESPONSE_SHAPE = [
    ...
    '<resource name="Health" value="80" min="0" max="100" description="..."/> <!-- max: plain number, OR a formula with Level / attribute names (e.g. "100+(Level*10)") for stats meant to grow with level; the description of anything that changes on its own states the drift rate (e.g. "rises 1 per scene; a meal resets it") -->',
...
const CHAR_PROMPT_HEADER = [
    ...
    "- DRIFT RATES: anything that changes ON ITS OWN as time passes (Hunger, Fatigue, rations, perishable items, clocks, fuel) MUST state in its description HOW MUCH it decays/increases and on what cadence — per message, scene, day or rest — plus what reverses or resets it ('Hunger rises 1 per scene; a full meal resets it to 0'). Values that only move through play (Health, Ammo, money) get no invented drift.",
    "- LEVEL: when the prompt states a level, use it; when progression is active WITHOUT a stated level, INFER it from the context (recent chat, details, reference levels) and report it in level (whole number, at least 1). Level 1 is ONLY for genuine beginners (fresh recruits, newly awakened heroes) — never default to it: a veteran, career soldier, established mage or mid-campaign joiner sits clearly higher. Stay at or below the LEVEL CAP given in the progression rules below. Without progression, omit the attribute.",
    "- SCALING RESOURCES: every character who fights, exerts or casts ALWAYS carries the core resources Health and Stamina, plus Mana when magic or powers fit their role — these anchors stay even when the rest of the sheet is minimal. When progression is active, at most 1-2 of these core resources may use a max formula mixing Level AND attribute names (e.g. \"100+(Level*10)\", \"80+(Endurance*25)\") instead of a plain number — everything else stays a plain number. Without progression, never use formulas.",
...
const CHAR_REFINE_PROMPT_HEADER = [
    ...
    "- REFERENCE CHARACTERS, when given, remain the template: keep mirroring their stat structure, resource names, ranges and granularity so similar characters stay comparable.",
    "- Without references, keep the container choice coherent: containers only where they matter for THIS character (a brute: resources+attributes; a mage: skills+passives; a quartermaster: inventory). Empty containers are correct when a system does not apply.",
    "- Resources are turn-to-turn meters updated during play (Health, Stamina, Ammo, Sanity, Stress) with sensible custom ranges. Attributes are milestone stats (Strength, Fortitude, Dexterity, Charisma) without hard caps, changed rarely.",
    "- CALIBRATE NUMBERS to the world: quantities and ranges must imply real scale. Anchor non-obvious scales in the description (e.g. 'a meal costs about 15').",
    "- SCALING RESOURCES: every character who fights, exerts or casts ALWAYS carries the core resources Health and Stamina, plus Mana when magic or powers fit their role — add any of these that are missing. When progression is active, at most 1-2 core resources may keep a max formula mixing Level AND attribute names (e.g. \"100+(Level*10)\", \"80+(Endurance*25)\") — everything else stays a plain number. Without progression, never use formulas.",
    ...
    "- DRIFT RATES: every resource/item that naturally decays/increases over time (Hunger, rations, perishables, clocks...) carries the rate IN ITS description — how much per message/scene/day/rest and what reverses or resets it. Fill in any that are missing or silent about drift.",
...
function progressionBlocks(targetLevel = null, isEnemy = false) {
    ...
    return [
        ...
        anchor,
        `LEVEL CAP: ${cap} — never report a level above it.`,
        ...(tiers ? [`TIER ANCHORS (world seniority ladder): ${tiers}`] : []),
    ];
}
