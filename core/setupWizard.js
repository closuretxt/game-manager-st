...
const SYSTEM_PROMPT_HEADER = [
    ...
    "- DRIFT RATES: anything that changes ON ITS OWN as time passes (Hunger, Fatigue, rations, perishable items, clocks, alert levels, fuel) MUST state in its description HOW MUCH it decays/increases and on what cadence — per message, scene, day or rest — plus what reverses or resets it ('Hunger rises 1 per scene; a full meal resets it to 0', 'Rations fall by 1 per day of travel'). Values that only move through play (Health, Ammo, money) get no invented drift.",
...
const REFINE_PROMPT_HEADER = [
    ...
    "- DRIFT RATES: every entry that naturally decays/increases over time (Hunger, rations, perishables, clocks, alert levels...) carries the rate IN ITS description — how much per message/scene/day/rest and what reverses or resets it. Fill in any that are missing, vague or silent about drift.",
    ...
    "- sharedResources stay party-wide and user-managed (money, food, expendables) — keep always_inject: true ONLY on the one whose value is relevant almost every turn (e.g. money); custom features stay AI-managed PARTY-WIDE entries — gimmicks, objectives, clocks, or hidden notes (secrets, plans, leads; visible in the Custom tab, never relationship/intimacy meters or per-character stats); warnings stay minimalist imminent-need remarks about the whole party, under 15 words.",
    "- If the scenario implies survival pressure (food, water, enemies, territory), keep it tangible through sharedResources + warnings for the group and per-character resources/statuses for individual states. If it is purely casual, keep the setup minimal.",
