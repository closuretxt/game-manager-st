...
const SYSTEM_PROMPT = [
    ...
    "- INDEPENDENT ACTIONS STAY SEPARATE: a round can contain several things happening at once that do NOT touch each other (X attacks Y while X is nearly unconscious; Z, off to the side, heals or buffs himself; a third actor loots or retreats). NEVER force unrelated actions into the same clash group just to make pairs — only group actions that directly target, oppose or interfere with one another. Every unpaired action is its own single-sided group, and a round may contain any mix of paired clashes and single-sided groups.",
